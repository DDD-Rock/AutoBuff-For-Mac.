...
运行前需要设置加血键和瞬移键，并在小地图上手动标记跟补基准点。确认点位时会同时保存小地图区域；运行时只使用该点的 X 坐标作为基准点，直接复用保存的小地图区域读取黄点。加血键每轮持续按住 10～15 秒，轮次之间只有短暂的拟人间隔。Buff 到期时会立刻松开加血键，优先按两次释放到期 Buff，并合并 10 秒内即将到期的 Buff，短暂等待后继续加血。

跟补模式只看横向位置，不处理纵向坐标。手动标记点的 X 会作为固定基准 X，基准左右约 6 个小地图点都算区域内。运行后每隔随机 10～13 秒，在加血键保持按下的同时，根据角色位于基准点的左侧或右侧，依次按住朝向基准点的方向键、短按瞬移键、松开瞬移键、松开方向键。黄点 X 走出基准区域时同样只朝基准点瞬移一次；瞬移后等待 500～800ms 让黄点稳定，即使落到基准点另一侧也不会立即反向瞬移。下一次瞬移只会由新的越界事件或下一个定时修正触发，整个过程不松开加血键。
...
- [ ] 跟补模式能每隔约 10～13 秒在持续加血时用方向键加瞬移向基准 X 修正
