...
运行前需要在小地图上手动标记跟补基准点，确认时会同时保存小地图区域；运行时只使用该点的 X 坐标作为基准点，直接复用保存的小地图区域读取黄点，随后持续释放加血技能。Buff 到期时会暂停补血，优先按两次释放到期 Buff，并合并 10 秒内即将到期的 Buff，释放结束后继续补血。

跟补模式只看横向位置，不处理纵向坐标。手动标记点的 X 会作为固定基准 X，基准左右约 6 个小地图点都算区域内。运行后每隔随机 12～15 秒做一次轻微站位修正：如果当前 X 偏离基准 X，就向基准 X 方向小走一段；如果已经在基准中心附近，则向右小走一步。单次修正默认按住方向键 200～300ms，界面中可调整。只有黄点 X 走出基准区域时，才会持续按住对应方向键走回区域内，到位后立刻松开方向键。
...
| `followHealAdjustMinMS/MaxMS` | 跟补模式周期站位修正方向键按住时长，单位毫秒 |
| `movementMode` | 活花移动模式 |
...
- [ ] 跟补模式能每隔约 12～15 秒向基准 X 中心轻微修正站位
- [ ] 跟补模式被碰撞或手动走出基准区域后，能持续按方向键回到区域内并松开
- [ ] 移动游戏窗口后，按钮点击位置仍正确
