...
跟补模式只看横向位置，不处理纵向坐标。手动标记点的 X 会作为固定基准 X，左右界限可在标记时配置。运行后每隔随机 4～7 秒，在加血键保持按下的同时，根据角色位于基准点的左侧或右侧，依次按住朝向基准点的方向键、短按瞬移键、松开瞬移键、松开方向键。系统会在用户界限的 75% 处提前保护；超过左侧硬界限时会绕过间隔和反向保护，立即向基准点强制回位。紧急回位后每 30～50ms 采样黄点，最少等待 150～250ms，连续两帧稳定后若仍在同侧界外会再次回位。普通瞬移跨到基准点另一侧时不会立即反向；越界回位也不会重置独立的 4～7 秒定时修正。
